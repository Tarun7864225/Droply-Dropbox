    ...
    try {
        ...
        const deletePromises = trashedFiles.filter((file) => !file.isFolder).map(async (file) => {
            try {
                ...
                if (imagekitFileId) {
                    try {
                        const searchResults = await imagekit.listFiles({name: imagekitFileId,limit: 1})
        ...
        const deletedFiles = await db.delete(files).where(and(eq(files.userId, userId), eq(files.isTrash, true))).returning();
        return NextResponse.json({success: true, message: `Successfully deleted ${deletedFiles.length} files from trash`});
